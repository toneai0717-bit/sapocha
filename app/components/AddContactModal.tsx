...
import { Camera, Sparkles, Plus, X } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg px-6 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] shadow-xl max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain">
        ...
        {/* スクショアップロード */}
        <div className="mb-4">
          <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 mb-2"><Camera className="w-3.5 h-3.5" aria-hidden="true" />プロフィールのスクショ（複数枚OK）</p>
          <div
            onDrop={handleDrop}
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  {previews.map((p, i) => (
                    <div key={i} className="relative">
                      ...
                      <button
                        ...
                        className="absolute -top-3 -right-3 w-11 h-11 rounded-full bg-slate-600 text-white flex items-center justify-center hover:bg-red-500 transition-colors"
                        aria-label="削除"
                      >
                  ...
                  <label className="w-16 h-16 rounded-lg border-2 border-dashed border-slate-200 flex items-center justify-center cursor-pointer hover:border-amber-300 transition-colors">
                    ...
                    <Plus className="w-5 h-5 text-slate-300" aria-hidden="true" />
                  </label>
                </div>
                <button
                  onClick={extractProfile}
                  disabled={extracting}
                  className="w-full inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-bold text-white bg-slate-700 hover:bg-slate-600 disabled:bg-slate-200 disabled:text-slate-400 transition-colors"
                >
                  {!extracting && <Sparkles className="w-4 h-4" aria-hidden="true" />}
