...
スクリーンショットまたはテキストの会話を読み取り、「相手（自分ではない方）の最新メッセージ」に対する返信案を3つ提案してください。
...
2. 絵文字：相手が使った絵文字の数と同じ数だけ使う。相手が0個なら0個、2個なら2個。
3. メッセージ分割：相手が2つのブロック（2回に分けて送信）しているなら、返信も「\n\n」で区切って2ブロックに分ける。1回なら1ブロック。
